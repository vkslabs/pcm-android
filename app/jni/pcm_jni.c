...
int main (argc, argv) int argc; char **argv;
{
    ...
    if (!overwrite && (outfile = fopen (outfilename, "r"))) {
        fclose (outfile);
        log_error("output file \"%s\" exists (use -y to overwrite)\n", outfilename);
        return -1;
    }
...
static int adpcm_converter (char *infilename, char *outfilename, int flags, int blocksize_pow2, int lookahead)
{
    ...
    if (!fread (&riff_chunk_header, sizeof (RiffChunkHeader), 1, infile) ||
        ...
        log_error("\"%s\" is not a valid .WAV file!\n", infilename);
        return -1;
    }
    ...
    while (1) {
        ...
        if (!strncmp (chunk_header.ckID, "fmt ", 4)) {
            ...
            if (!supported) {
                log_error("\"%s\" is an unsupported .WAV format!\n", infilename);
                return -1;
            }

            if (verbosity > 0) {
                log_debug("format tag size = %d\n", chunk_header.ckSize);
                log_debug("FormatTag = 0x%x, NumChannels = %d, BitsPerSample = %d\n",
                         WaveHeader.FormatTag, WaveHeader.NumChannels, WaveHeader.BitsPerSample);
                log_debug("BlockAlign = %d, SampleRate = %d, BytesPerSecond = %d\n",
                         WaveHeader.BlockAlign, WaveHeader.SampleRate, WaveHeader.BytesPerSecond);

                if (chunk_header.ckSize > 16) {
                    if (format == WAVE_FORMAT_PCM)
                        log_debug("cbSize = %d, ValidBitsPerSample = %d\n", WaveHeader.cbSize,
                                 WaveHeader.Samples.ValidBitsPerSample);
                    else if (format == WAVE_FORMAT_IMA_ADPCM)
                        log_debug("cbSize = %d, SamplesPerBlock = %d\n", WaveHeader.cbSize,
                                 WaveHeader.Samples.SamplesPerBlock);
                }

                if (chunk_header.ckSize > 20)
                    log_debug("ChannelMask = %x, SubFormat = %d\n",
            ...
            if (!chunk_header.ckSize) {
                log_error("this .WAV file has no audio samples, probably is corrupt!\n");
                return -1;
            }
            ...
            if (format == WAVE_FORMAT_PCM) {
                ...
                if (leftover_bytes) {
                    ...
                    if (verbosity > 0) log_debug("data chunk has %d bytes left over for final ADPCM block\n", leftover_bytes);
                ...
                if (fact_samples) {
                    if (fact_samples < num_samples && fact_samples > num_samples - samples_last_block) {
                        if (verbosity > 0) log_debug("total samples reduced %lu by FACT chunk\n", (unsigned long) (num_samples - fact_samples));
                        ...
                        if (verbosity > 0) log_debug("num samples reduced %lu by [incorrect] FACT chunk\n", (unsigned long) (num_samples - fact_samples));
                        num_samples = fact_samples;
                    }
                }
            }
            ...
            if (verbosity > 0)
                log_debug("extra unknown chunk \"%c%c%c%c\" of %d bytes\n",
    ...
    if (format == WAVE_FORMAT_PCM) {
        ...
        if (verbosity > 0)
            log_debug("each %d byte ADPCM block will contain %d samples * %d channels\n",
        ...
        if (!(flags & ADPCM_FLAG_RAW_OUTPUT) && !write_adpcm_wav_header (outfile, num_channels, num_samples, sample_rate, samples_per_block)) {
            log_error("can't write header to file \"%s\" !\n", outfilename);
            return -1;
        }

        if (verbosity >= 0) log_debug("encoding PCM file \"%s\" to%sADPCM file \"%s\"...\n",
        ...
        if (verbosity >= 0) log_debug("decoding ADPCM file \"%s\" to%sPCM file \"%s\"...\n",
                                     infilename, (flags & ADPCM_FLAG_RAW_OUTPUT) ? " raw " : " ", outfilename);
...
static int adpcm_decode_data (FILE *infile, FILE *outfile, int num_channels, size_t num_samples, int block_size)
{
    ...
    if (!pcm_block || !adpcm_block) {
        log_error("could not allocate memory for buffers!\n");
        return -1;
    }

    if (verbosity >= 0 && num_samples > 1000) {
        progress_divider = (num_samples + 50) / 100;
        update_progress(percent = 0);
        fflush (stderr);
    }
    ...
    while (num_samples) {
        ...
        if (!fread (adpcm_block, block_size, 1, infile)) {
            log_error("could not read all audio data from input file!\n");
            return -1;
        }

        if (adpcm_decode_block (pcm_block, adpcm_block, block_size, num_channels) != this_block_adpcm_samples) {
            log_error("adpcm_decode_block() did not return expected value!\n");
            return -1;
        }

        if (!fwrite (pcm_block, this_block_pcm_samples * num_channels * 2, 1, outfile)) {
            log_error("could not write all audio data to output file!\n");
            return -1;
        }
        ...
        if (progress_divider) {
            ...
            if (new_percent != percent) {
                update_progress(percent = new_percent);
                fflush (stderr);
            }
        }
    }
    ...
    if (verbosity >= 0)
        log_debug("\r...completed successfully\n");
...
static int adpcm_encode_data (FILE *infile, FILE *outfile, int num_channels, size_t num_samples, int samples_per_block, int lookahead, int noise_shaping)
{
    ...
    while (num_samples) {
        ...
        if (!fread (pcm_block, this_block_pcm_samples * num_channels * 2, 1, infile)) {
            log_error("\rcould not read all audio data from input file!\n");
            return -1;
        }
        ...
        if (num_bytes != block_size) {
            log_error("\radpcm_encode_block() did not return expected value (expected %d, got %d)!\n", block_size, (int) num_bytes);
            return -1;
        }

        if (!fwrite (adpcm_block, block_size, 1, outfile)) {
            log_error("\rcould not write all audio data to output file!\n");
            return -1;
        }
...
JNIEnv* envObject = NULL;
jobject  listenerObject = NULL;


void update_progress(int value) {
    if(envObject && listenerObject) {
        jclass cls = (*envObject)->GetObjectClass(envObject, listenerObject);
        jmethodID method_getValue = (*envObject)->GetMethodID(envObject, cls, "updateProgress","(I)V");
        (*envObject)->CallVoidMethod(envObject, listenerObject, method_getValue, value);
    }
}


JNIEXPORT jboolean JNICALL
Java_com_vkslabs_pcm_decoder_PCMDecoder_decodeToPCM( JNIEnv* env,
                                                  jobject thiz, jstring inputFilePath, jstring outputFilePath, jobject listener) {
    int lookahead = 3, flags = ADPCM_FLAG_NOISE_SHAPING, blocksize_pow2 = 0;
    const char *inputFilePathString = (*env)->GetStringUTFChars(env, inputFilePath, 0);
    const char *outputFilePathString = (*env)->GetStringUTFChars(env, outputFilePath, 0);
    envObject = env;
    listenerObject = listener;
    __android_log_print(ANDROID_LOG_DEBUG, TAG, "Decoding Started...");
    int result = adpcm_converter (inputFilePathString, outputFilePathString, flags, blocksize_pow2, lookahead);
    __android_log_print(ANDROID_LOG_DEBUG, TAG, "Decoding Completed...");
    (*env)->ReleaseStringUTFChars(env, inputFilePath, inputFilePathString);
    (*env)->ReleaseStringUTFChars(env, outputFilePath, outputFilePathString);
    envObject = NULL;
    listenerObject = NULL;

    if(result == -1) {
        jclass exception = (*env)->FindClass(env, "Exception");
